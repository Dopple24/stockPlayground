...
stockData = yf.Tickers(" ".join(tickers_list))
...
for t_symbol, t_obj in stockData.tickers.items():
    try:
        ...
        earnings_yield = (eps / price * 100 / 10) if (eps and price) else None
        ...
        # FCF Yield (/10)
        fcf_yield = (fcf / market_cap * 100 / 10) if (fcf and market_cap) else None

        all_data.append({
            "Ticker": t_symbol,
            "Price": price,
            "Earnings Yield (/10)": earnings_yield,
            "PEG": peg,
            "Debt/Equity": de_ratio,
            "Revenue Growth (/10)": rev_growth,
            "FCF Yield (/10)": fcf_yield
        })
